...
net = vgg(model_name='vgg13', num_classes=2, init_weights=False)
...
def train(net, train_iter, num_epochs, lr, device):
    ...
    torch.save(min_loss_state_dict, 'VGG13.pth')
...
with torch.no_grad():
    for k in range(len(a)):
        ...
        for images, label8 in test_loader:
            images, label8 = images.to(device), label8.to(device)
            net.load_state_dict(torch.load('VGG13.pth'))
            net.to(device)
            outputs8 = net(images)
        ...
        if k == 0:
            ...
            plt.savefig(f"VGG13_ROC曲线图.png")
