...
net = se_resnet18(num_classes=2)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
train_dataset = TensorDataset(train_images1, train_label08)  #将数据进行封装
...
def train(net, train_iter, num_epochs, lr, device):
    ...
    torch.save(min_loss_state_dict, 'SE_ResNet18.pth')
...
with torch.no_grad():
    for k in range(len(a)):
        ...
        for images, label8 in test_loader:
            images, label8 = images.to(device), label8.to(device)
            net.load_state_dict(torch.load('SE_ResNet18.pth'))
            net.to(device)
            outputs8 = net(images)
        ...
        if k == 0:
            ...
            plt.savefig(f"SE_ResNet18_ROC曲线图.png")
