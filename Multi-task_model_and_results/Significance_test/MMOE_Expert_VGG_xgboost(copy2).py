...
with torch.no_grad():
    ...
    for images, label0, label1, label2, label3, label4, label5, label6, label7, label8 in test_loader:
        ...
        y_labels5.extend((0.3 * y_pred1 + 0.5 * y_pred2 + 0.2 * y_pred3))
...
# 各模型的预测概率转换为二分类结果
models = [
    (np.array(y_labels1), "MMOE+xgboost"),
    (np.array(y_labels2), "MMOE+lightgbm"),
    (np.array(y_labels3), "MMOE+gbdt"),
    (np.array(y_labels4), "MMOE"),
    (np.array(y_labels5), "MDT-TC")
]

# 生成预测类别（0或1）
y_preds = []
for pred_prob, name in models:
    y_pred = (pred_prob >= 0.5).astype(int)
    y_preds.append((y_pred, name))

# 进行所有模型两两比较
model_pairs = list(combinations(y_preds, 2))

results = []
for (modelA_pred, modelA_name), (modelB_pred, modelB_name) in model_pairs:
    # 构建列联表
    a = np.sum((modelA_pred == y_true) & (modelB_pred == y_true))  # 两者正确
    b = np.sum((modelA_pred == y_true) & (modelB_pred != y_true))  # A对B错
    c = np.sum((modelA_pred != y_true) & (modelB_pred == y_true))  # A错B对
    d = np.sum((modelA_pred != y_true) & (modelB_pred != y_true))  # 两者错误
    ...
    results.append({
        'Model Pair': f"{modelA_name} vs {modelB_name}",
        'Disagreement (B)': b,
        'Disagreement (C)': c,
...
# 输出结果
print("McNemar检验结果（显著性水平α=0.05）:")
print("=============================================")
for res in results:
    sig = "显著" if res['p-value'] < 0.05 else "不显著"
    print(f"{res['Model Pair']}:")
    print(f"  不一致情况 B={res['Disagreement (B)']}, C={res['Disagreement (C)']}")
    print(f"  统计量={res['Statistic']:.3f}, p值={res['p-value']:.4f} ({sig})\n")
